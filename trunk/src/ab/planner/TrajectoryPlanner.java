...
public class TrajectoryPlanner {
    ...
    /*
     * function to check if  Math.sqrt(v4 - (x * x + 2 * y * v2)) is negative
     * means that a Node is infeasible
     * added by AngryBER to check Nodes' feasibility
    */
    public boolean feasibleNodeAngle(Rectangle sling, Point tpt){
    	
        double scale = getSceneScale(sling);
        Point ref = getReferencePoint(sling);
            
        double x = (tpt.x - ref.x) / scale;
        double y = -(tpt.y - ref.y) / scale;
        
        double v = _scaleFactor * _launchVelocity[6];
        double v2 = v * v;
        double v4 = v2 * v2;
        
        double res = v4 - (x * x + 2 * y * v2);
    ...
    public void setTrajectory(Rectangle sling, Point releasePoint)
    {
        ...
        if (_trajSet &&_ref != null && _ref.equals(getReferencePoint(sling)) && _release != null && _release.equals(releasePoint))
            return;
    ...
    public int getTimeByDistance(Rectangle sling, Point release, Point tapPoint)
    {
        ...
        //predictTrajectory(sling, release);
        ...
        //System.out.println("getTimeByDistance(): " + (int)(distance / _ux * _timeUnit));
	...
	public int getTapTime(Rectangle sling, Point release, Point target, int tapInterval)
	{
		...
		//System.out.println("TapPoint: " + tapPoint);
